...
Link.prototype = {
    ...
    _insertLink: function _insertLink(text, link,encode) {
        ...
        if (checkResult === true) {
            if(encode)
            editor.cmd.do('insertHTML', '<a href="'+link+'">'+text+'</a>');
...
Code.prototype = {
    ...
    _createPanel: function _createPanel(value) {
        ...
        if (editor.customConfig.codeType != null) {
            var select = "<div class='codeType' style='margin-bottom: 10px;'><select class='uk-select' style='width:calc(100% - 20px);padding: 0 10px'>";
        ...
        var panel = new Panel(this, {
            ...
            tabs: [{
                ...
                title: '插入代码',
                // 模板
                tpl:'<div>' + select + '<textarea id="' + textId + '" style="height:145px;;">' + value +
                    '</textarea>' +
                    '<div class="w-e-button-container">' +
...
UploadImg.prototype = {
    ...
    insertLinkFile: function insertLinkFile(title,link) {
        ...
        editor.cmd.do('insertHTML', '<a href="'+link+'" >'+title+'</a>');
    ...
    // 上传图片
    uploadFile: function uploadFile(files,url) {
        var _this3 = this;
        ...
        var editor = this.editor;
        var config = editor.config;
        var uploadImgServer = url ? url : config.uploadImgServer;
        var uploadImgShowBase64 = config.uploadImgShowBase64;
