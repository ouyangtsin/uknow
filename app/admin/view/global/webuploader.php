<script type="text/javascript">
    ...
    function webupload(list, filePicker_image, image_preview, image, more, upload_allowext, size, type) {
        ...
        var uploader = WebUploader.create({
            ...
            server: '{:url("api/upload/index")}' + '?upload_type=' + type, // 文件接收服务端
        ...
        uploader.on('uploadSuccess', function (file, response) {
            if (response.code == 0) {
                UK.modal.alertError(response.msg);
            }
        ...
        uploader.on("error", function (type) {
            if (type == "Q_TYPE_DENIED") {
                UK.modal.alertError('请上传' + upload_allowext + '格式的文件！');
            } else if (type == "F_EXCEED_SIZE") {
                UK.modal.alertError('单个文件大小不能超过' + size / 1024 + 'kb！');
            } else if (type == "F_DUPLICATE") {
                UK.modal.alertError('请不要重复选择文件');
            } else {
                UK.modal.alertError('上传出错！请检查后重新上传！错误代码' + type);
            }
        });
    }
</script>