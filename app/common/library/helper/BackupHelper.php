...
class BackupHelper
{
    ...
    public static function getInstance($config = []): BackupHelper
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\\d{8,8}-\\d{6,6}-\\d+\\.sql(?:\\.gz)?$/', $name)) {
                ...
                if (isset($list["{$date} {$time}"])) {
                    ...
                    $info['size'] = formatBytes($info['size'] + $file->getSize());
                } else {
                    $info['part'] = $part;
                    $info['size'] = formatBytes($file->getSize());
                }
    ...
    {
        if ($time) {
            $file = $this->getFile('time', (int)$time);
            array_map("unlink", $this->getFile('time', $time));
            if (count($this->getFile('time', (int)$time))) {
     ...
     * @throws \Exception
     */
    public function downloadFile(int $time, $part = 0)
    {
     ...
     * @return mixed     true - 写入成功，false - 写入失败！
     */
